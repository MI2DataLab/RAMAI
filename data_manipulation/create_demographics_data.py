...
games_dpm = pd.read_csv(os.path.join(RAW_CSV_DIR, f"dpm_games.csv"))
games_dpm["group"] = "dpm"
games_mlinpl = pd.read_csv(os.path.join(RAW_CSV_DIR, f"mlinpl_games.csv"))
games_mlinpl["group"] = "mlinpl"

games = pd.concat([games_dpm, games_mlinpl], ignore_index=True)
...
demographics.to_csv(os.path.join(DEMOGRAPHICS_CSV_DIR, "demographics.csv"), index=False)
